...
public class PickUp : MonoBehaviour
{
    ...
    [SerializeField] private AudioClip throwingStoneSound;
    ...
    private void ThrowItem()
    {
        if (currentItem != null)
        {
            ...
            // Get the player's horizontal speed (affects throw force)
            float playerSpeed = Mathf.Abs(GetComponent<Rigidbody2D>().velocity.x);

            // Calculate the throw force based on the player's speed and the item's mass
            float throwForce = (playerSpeed * baseThrowForce) * itemRb.mass;
            ...
            // Play the throwing sound once its thrown
            SoundManager.instance.PlaySound(throwingStoneSound);
